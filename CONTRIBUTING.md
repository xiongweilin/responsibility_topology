...
- an unsupported inference from research dormancy to search saturation or ontological absence.
...
If a field was never recorded, write `UNRECORDED`. Do not reconstruct exact queries, budgets, or database coverage after the fact and present them as contemporaneous provenance.
...
A governance/resource pause may be legitimate without saturation. In that case say so explicitly and leave `RealityHypothesis = UNRESOLVED`.
