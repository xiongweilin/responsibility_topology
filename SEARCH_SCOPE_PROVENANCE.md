...
A future claim that active search is sufficiently complete to justify suspension on epistemic grounds must contain a `SearchScopeRecord`.
...
```text
all other institutional/scientific/legal/technical mechanisms: NOT EXHAUSTIVELY SEARCHED
exact search-engine/database/query coverage: UNRECORDED
search budget: UNRECORDED
```
...
### MechanismFamiliesKnownButNotSearched / blind spots
...
```text
empirical saturation of mechanism-distinct Candidate-A-like cases;
absence of a second independent provenance mechanism in reality;
adequacy of the search vocabulary for discovering unknown mechanisms.
```
...
independent search-strategy replication: NOT ESTABLISHED
marginal novelty curve: NOT RECORDED
```
...
### 9.6 Historical fields not recorded / blind spots
