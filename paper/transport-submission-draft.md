...
Status: Paper 2 claim-frozen draft after hostile review. Venue formatting, bibliography style, anonymization, and final title remain unfrozen.
...
A judgment that crosses a representation or context boundary acquires at least two different kinds of historical dependency: dependency on the judgment being translated and dependency on a separately recorded translation witness. A stateful system adds a further distinction. The historical parents that explain how a target judgment was formed need not be the coordinates at which those parents are currently usable. Conflating these relations makes it difficult to state what a translation step records, what a later qualification step must check, and which dependencies belong to translated content rather than to translation evidence.
...
We prove four result families. First, TRANSPORT formation has an exact canonical historical shape and enforces the kernel's modeled scope and escalation-strength restrictions. Second, qualification is source-indexed: current responsibility follows the stored parent identities back to their own formation contexts. Third, translation evidence is isolated in lineage: away from `BRIDGE`, transported ancestry is exactly the original ancestry. Fourth, these historical conservation laws compose across two adjacent TRANSPORT formation steps linked by the exact intermediate child identity: scope and canonically interpretable escalation depth do not amplify, non-`BRIDGE` ancestry remains exact, and both translation witnesses accumulate only under `BRIDGE` responsibility.

The contribution is not a new general theory of proof transfer, contextual logic, ontology mapping, provenance transformation, delegation, or relabeling. Those areas have substantial prior work. The narrower result is a machine-checked responsibility decomposition showing that, in the represented same-profile kernel, origin identity, translation-evidence responsibility, target historical identity, and source-indexed current qualification can remain distinct across a context/representation boundary.

---

## 1. Introduction

Systems that transform judgments across representations often use a single word—*translation*, *mapping*, *transport*, *conversion*—for several logically different responsibilities. Suppose a judgment `O` is expressed in context `c_A`, a bridge witness `X_1` records an exact binding to a map, source identity, target context, and translated claim, and the system forms a new judgment `W_1` in context `c_B`. At least three historical facts are now present. `W_1` was formed from `O`; the formation also depended on `X_1`; and `W_1` is a distinct target historical object rather than a mutation of `O`.
...
The second issue is conservation. If a judgment is transported from `c_A` to `c_B`, and the resulting historical child is transported again from `c_B` to `c_C`, what represented properties can be shown to survive both crossings? We focus on three. Scope may only narrow. Canonically interpretable escalation depth may not increase. Most importantly, translation evidence must not masquerade as the original content/source ancestry: non-`BRIDGE` lineage remains exact, while witness ancestry accumulates only under `BRIDGE` responsibility. These claims are about the kernel's represented history; they do not establish semantic adequacy of the translation maps or current usability propagation through the chain.
...
`O` is the original historical warrant. `X_1` is the `BRIDGE` witness whose claim binds the first map, original identity, target, and translated claim. Formation produces `W_1` at `c_B`. A second witness `X_2` is used in formation of `W_2` at `c_C`, with `W_1` used as the exact original identity of the second TRANSPORT event.
...
We therefore do not claim novelty for representation crossing, bridge rules, proof transport, provenance-preserving transformation, attenuation, trust translation, or interoperability in general. The contribution is the represented responsibility cut and its mechanized conservation results inside this same-profile kernel.
...
- semantic adequacy, truth, or interoperability correctness of translation maps or bridge witnesses.
...
Define the semantic translation witness `transportableClaim(map, original, target, translatedClaim)` at the level of structured claims. Explicitly state that this is a kernel-semantic binding and not Python's JSON `Claim.key()` encoding or a theorem that the translation is semantically adequate.
...
target's modeled accepts predicate holds for translated claim
...
The target `accepts` field is a represented formation predicate. It does not mean that the target context is active, adopted, licensed, or entitled. No current usability or target activation premise appears here.
...
Do not describe this conceptual distinction as a machine-proved pairwise non-implication theorem.
...
be one kernel step. Then there exist canonical binding `B`, target context `C_t`, original `O`, and witness `X` such that the pre-state contains those exact referents, the represented transport formation discipline holds, and:
...
Immediately unpack only the paper-relevant consequences: target formation context, same profile digest, original role, exact ordered parents, modeled scope restrictions, canonical-interpretable strength restriction, and lineage transform.
...
Formation records the historical explanation represented by the kernel. It does not decide whether the target judgment is currently usable, adopted, entitled, true, or semantically adequate.
...
Emphasize that these coordinates come from the historical parent objects, not from the target child context. The theorem does not require the two source contexts to be equal to one another or to the target, and it does not require any of those contexts to be active.
...
Qualification reads immutable child history only far enough to identify the TRANSPORT constructor, exact two stored parents, profile, and target context. It does not replay formation checks such as exact translation binding, the target's modeled acceptance predicate, scope, strength, or lineage. “Does not replay” means the premise is absent from the later transition; it does not mean the historical formation fact is unimportant or invalid.
...
Do not generalize this to arbitrary intervening transitions. The theorem establishes no persistence property after `sigma_2` and no entitlement consequence from `Usable`.
...
Interpretation: translation-witness dependency is recorded, but its responsibility category is not allowed to masquerade as the original role-indexed ancestry.
...
This is stronger and more precise than saying vaguely that “provenance is preserved,” a phrase the paper should avoid because provenance through transformations is an established broader literature.

### 5.3 BRIDGE is responsibility, not truth

Clarify that BRIDGE ancestry records dependency on a translation witness. It does not establish truth, semantic equivalence, authorization, target adoption, or adequacy of the witness.
...
This is a theorem about the represented `ScopeNarrowerOrEqual` relation. Do not reinterpret it as a general authorization, delegation, least-privilege, information-flow, or trust theorem.
...
Do not describe this as a total numeric semantics for all claims or as a general notion of trust/authority attenuation.
...
This is historical lineage composition. It is not a theorem that `W_1` or `W_2` is currently usable, valid, adopted, or semantically equivalent to `O`.
...
The reachable theorem does not re-expand the formation discipline into a second theorem surface and contains no `qualifyTransport` premise or conclusion.

### 6.6 Why two hops are the stopping point

Two hops provide a machine-checked composition result and show that the stated conservation properties hold across the represented two-step boundary. The current manuscript does not claim arbitrary-length closure and does not need an arbitrary-length chain datatype, fold, witness sequence, or induction theorem. Those would be reopened only if a concrete indispensable paper argument required arbitrary `n`.
...
Acknowledge Distributed Description Logics, C-OWL, and related mapped-context systems. Make explicit that this paper's `BRIDGE` is a lineage responsibility role, not a claim to invent bridge-rule reasoning or cross-context interoperability.
...
Conclude related work with the four-way responsibility decomposition and the two-hop conservation theorem; make no priority claim and no claim that the four-way decomposition is empirically or semantically adequate outside the represented kernel.
...
Both original and witness must match the current binding's profile digest. Cross-profile transport would require a new explicit responsibility mechanism and is outside this paper. “Cross-context” in this paper must not be paraphrased as arbitrary interoperability.

### 8.2 Transportability is not target adoption

The kernel does not require target context activation/adoption during formation or qualification. A candidate target context can therefore participate in represented historical exploration without the paper asserting that the context is actionably adopted.

### 8.3 Qualification is not entitlement

Current `Usable` is a kernel currentness predicate. The paper makes no implication from usability to entitlement, authorization, truth, or semantic adequacy.
...
No path datatype or `n`-hop induction result is claimed. The phrase “compositional” should refer only to the proved two-hop composition unless a later theorem explicitly generalizes it.

### 8.7 Semantic kernel is not executable refinement

The Python reference implementation motivates and tests related behavior, but the paper does not claim a formal refinement theorem connecting Python's container/serialization semantics to the Lean transition system. The kernel-semantic `transportableClaim` must not be identified with Python JSON bytes.
...
Representation crossing introduces responsibilities that should not be collapsed merely because they participate in one operational transition. In the same-profile kernel studied here, TRANSPORT records an exact target historical judgment while retaining the original and bridge-witness identities. A later qualification step returns to those stored historical parents' own formation contexts to discharge the kernel's current-usability responsibility. The lineage semantics separately preserve original non-`BRIDGE` ancestry and confine translation-witness ancestry to `BRIDGE` responsibility.

These distinctions remain stable across the two consecutive historical TRANSPORT formations proved in P2-R4: scope and canonically interpretable escalation strength do not amplify, non-`BRIDGE` ancestry remains exact, and each translation witness contributes only to BRIDGE ancestry. The result is a finite mechanized account of responsibility preservation across a represented context/representation boundary—not a general interoperability theory, and not a claim that translation, provenance transformation, delegation, or proof transport are new.
...
## Architecture freeze after hostile review

The paper is frozen to this high-level order unless a concrete claim-surface ambiguity requires reorganization:
...
Paper 1 should occupy only the minimum background needed to explain why historical state and current usability are represented separately. Paper 2's independent question is how that separation behaves when a judgment crosses a same-profile context/representation boundary.

No new formal phase is opened by the current manuscript.
