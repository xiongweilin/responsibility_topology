...
class ActivationObservation:
    ...
    def bootstrap() -> "ActivationObservation":
        return ActivationObservation("bootstrap")
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "bootstrap" and self.license_id is not None:
            raise ValueError("Bootstrap activation cannot carry a license")
        if self.kind == "adopt" and self.license_id is None:
            raise ValueError("Adopt activation requires a license")
...
) -> Mapping[ContextKey, ActivationObservation]:
    observations = {}
    for raw_key, license_id in state.context_activation_license.items():
        key = ContextKey.from_state_key(raw_key)
        observations[key] = (
            ActivationObservation.bootstrap()
            if license_id is None
            else ActivationObservation.adopt(license_id)
        )
...
def capture_pre_refresh_boundary(state: EvaluationState) -> PreRefreshBoundary:
    ...
    active = MappingProxyType({
        key: state.context_status(key.binding, key.context, key.use)
        == ContextStatus.ACTIVE
        for key in ordered
    })
    return PreRefreshBoundary(ordered, active, activation)


def _clone_state(state: EvaluationState) -> EvaluationState:
    """Conformance-only exact copy of mutable evaluation observations."""
...
def issuer_context_is_active(
    ...
    return state.context_status(
        issuer.binding, issuer.context, issuer.use
    ) == ContextStatus.ACTIVE
...
def check_license_base_current(
    ...
    """Observe all currentness premises except issuing-context activity.

    The adapter does not reproduce `check_license_current`.  It copies the
    concrete post-state, neutralizes only the exact issuing-context activity
    premise, then delegates all remaining checks to the reference kernel.
    """
    issuer = issuer_context_for_license(history, license_id)
...
def capture_context_refresh_snapshot(
    ...
    python_active = MappingProxyType({
        key: state_after.context_status(key.binding, key.context, key.use)
        == ContextStatus.ACTIVE
...
def certify_python_currentness(
    ...
    for context in snapshot.context_keys:
        if snapshot.python_active_after[context]:
            results.append(CertifiedCurrentnessResult(
                context=context,
                python_active=True,
...
# ============================================================
# Concrete Lean fixture encoding
# ============================================================
...
def _prop_membership(variable: str, values: Sequence[str]) -> str:
    return "False" if not values else " ∨ ".join(
        f"{variable} = {value}" for value in values
    )
...
def _issuer_function(snapshot: ContextRefreshSnapshot) -> str:
    ...
    return (
        "def issuerFn (l : ActivationLicenseId) : Option ContextKey :=\n"
        f"  {body}\n"
    )
...
def _lean_grounded_certificate(cert: GroundedCertificate) -> str:
    ...
    if (
        cert.kind != "adopt"
        or cert.license_id is None
        or cert.issuer is None
        or cert.tail is None
    ):
...
def _simp_line(checker: str, context_name: str, cert_name: str) -> str:
    return (
        "  simp ["
        f"{checker}, R, {context_name}, {cert_name}, "
        "seedActiveFn, activationFn, issuerFn, baseCurrentFn"
        "]"
    )
...
def build_currentness_lean_fixture(snapshot: ContextRefreshSnapshot) -> str:
    ...
    for index, result in enumerate(certified):
        context_name = f"c{index}"
        cert_name = f"cert{index}"
        lines.append(
            f"def {context_name} : ContextKey := {_lean_context_key(result.context)}"
        )
        if result.python_active:
            ...
            lines.extend([
                f"example : Grounded R {context_name} := by",
                f"  apply groundedCertificate_sound (cert := {cert_name})",
                _simp_line(
                    "CheckGroundedCertificate", context_name, cert_name
                ),
            ])
        else:
            ...
            lines.extend([
                ...
                _simp_line(
                    "CheckUngroundedCertificate", context_name, cert_name
                ),
            ])
        lines.append("")
